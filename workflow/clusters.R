...
if (length(i) > 0) {
  ...
  oGr = CGraphClust.recalibrate(oGr, iVertKeep, clusterMethod = cluster_leading_eigen)
}
...
dim(mCounts)
mMarginal = getClusterMarginal(oGr, t(mCounts), bScaled = T)
dim(mMarginal)
str(mMarginal)
...
m1 = t(scale(m1))
dim(m1)
colnames(m1) = as.character(fGroups)
...
## revisit the stabalization sections
...
pr.comp = prcomp(t(mMarginal), center = T, scale = T)
plot(pr.comp$x, col=ivCols, pch=20, main='Analytical')
text(pr.comp$x, labels = rownames(mCounts), adj = 1, cex = 0.2)
legend('bottomright', legend = levels(fGroups), fill = rainbow(n = nlevels(fGroups)))
plot(pr.comp$sdev)

mComp.all = t(mMarginal)
#mComp.all = pr.comp$x[,1:3]
dim(mComp.all)
m = mComp.all
rownames(m) = as.character(fGroups)
d = dist(m)
plot(hclust(d))
d2 = as.matrix(d)
dim(d2)
d2 = d2[nrow(d2):1,]
image(1:ncol(d2), 1:ncol(d2), d2, axes=F, xlab='', ylab='', col=grey.colors(5))
axis(1, 1:ncol(d2), colnames(d2), cex.axis=0.2, las=3)
axis(2, 1:ncol(d2), rownames(d2), cex.axis=0.2, las=1)

rownames(m) = colnames(mMarginal)
k = kmeans(m, 4)
k$cluster
...
dim(d2)
image(1:ncol(d2), 1:ncol(d2), d2, axes=F, xlab='', ylab='', col=grey.colors(6))
axis(1, 1:ncol(d2), colnames(d2), cex.axis=0.5, las=3)
axis(2, 1:ncol(d2), rownames(d2), cex.axis=0.4, las=1)

plot.mean.expressions(oGr, t(mCounts), fGroups)

...
# Date: 11/03/2016
# Desc: merge the two tb datasets
...
f_plot.diagnostics(ig.merge, tb_data$matrix, tb_data$groups)

# keep genes common between two tb datasets
...
vcount(igi)
cVertID = V(igi)$name
ig.tb.long = CGraphClust.recalibrate(tb_data$graph, cVertID)
ig.ltb = CGraphClust.recalibrate(ltb_atb_data$graph, cVertID)
f_plot.diagnostics(ig.tb.long, tb_data$matrix, tb_data$groups)
f_plot.diagnostics(ig.ltb, ltb_atb_data$matrix, ltb_atb_data$groups)

# remove genes common between the two tb datasets
ig.tb.long = getFinalGraph(tb_data$graph)
ig.ltb = getFinalGraph(ltb_atb_data$graph)
# get vertices not common 
iVertID.ltb = which(!(V(ig.ltb)$name %in% V(ig.tb.long)$name))
# get the graph
ig.ltb = CGraphClust.recalibrate(ltb_atb_data$graph, iVertID.ltb)
f_plot.diagnostics(ig.ltb, ltb_atb_data$matrix, ltb_atb_data$groups)

## save the graph of interest as object 
ltb_atb_unique = list(graph=ig.ltb, matrix=ltb_atb_data$matrix, groups=ltb_atb_data$groups)
save(ltb_atb_unique, file='Objects/ltb_atb_unique.rds')
